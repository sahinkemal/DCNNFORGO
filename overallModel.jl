...
#keep the input size same in each layer, following script is a overall network example without
#input data
using Knet
...
@knet function GoDCNNModel(x)
    ...
    y4 =cbf(y3,out=48,cwindow=5,cpadding=2);
    y5 =cbf(y4,out=32,cwindow=5,cpadding=2);
    y6 =cbf(y5,out=32,cwindow=5,cpadding=2);
    return wbf(y6; out=19*19, f=:soft);
end
